...
class SmoothGreedy:
    def __init__(self, A, Y, data=None, k=12, eps=0.01):
        ...
        self.n = Y.shape[1]  # number of vectors # M: this should be Y.shape[0] given current input in main
        self.d = A.shape[1]  # dimension of vectors # M: this should be A.shape[0] given current input in main
        self.m = A.shape[0]  # dimension of measurements # M: this should be A.shape[1] given current input in main
    ...
    def compute_reconstruction(self):
        for i in range(self.n):  # TODO: do this in parallel?
            if i % 50 == 0:
    ...
    def compute_best_indices(self, i):
        ...
        remaining = set(range(self.d))
        measurement = Y[:, i]  # M: this should be Y[i, :]
        l0 = 1/2*np.inner(measurement, measurement)
        current_obj_val = 0
        best_x = None
        chosen = []
        ...
        for _ in range(self.k):
            ...
            for j in remaining:
                ...
                best_x, loss = self.r1_find_best_x(j, current_indices, measurement, A_sub, pseudo_inv)
                objective[j] = l0 - loss
                g[j] = objective[j] - current_obj_val
...
if __name__ == "__main__":
    ...
    sg = SmoothGreedy(A, Y, data, k=20)  # M: either pass A.T and Y.T or change SmoothGreedy accordingly
    rmse = sg.evaluate()
    print("RMSE: ", rmse)
